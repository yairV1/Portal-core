...
<div class="doc-hero">
  <h1 class="doc-hero-titulo">Hojas de vida</h1>
  <p class="doc-hero-desc">Hoja de vida por empleado, con su archivo adjunto.</p>
</div>
...
<?php else: ?>
  <table class="table">
    <thead><tr><th>Nombre</th><th>Documento</th><th>Cargo</th><th>Hoja de vida</th><th></th></tr></thead>
    <tbody>
        ...
        <tr>
          ...
          <td>
            <?php if ($emp['doc_id'] && $emp['doc_archivo']): ?>
              ...
              <span class="text-muted" style="font-size:12px"> · <?= e((new DateTime($emp['fecha']))->format('d/m/Y')) ?></span>
              <a class="doc-btn doc-btn--secondary doc-btn--icon" href="<?= BASE_URL ?>/talento-humano/hojas-de-vida/descargar?id=<?= (int) $emp['doc_id'] ?>" target="_blank" rel="noopener" title="Descargar"><i class="bi bi-download"></i></a>
              <?php if ($puedeAdministrar): ?>
                <form action="<?= BASE_URL ?>/talento-humano/hojas-de-vida/eliminar" method="post" style="display:inline-block" onsubmit="return confirm('¿Eliminar esta hoja de vida?')">
                  <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            ...
            <?php elseif ($puedeAdministrar): ?>
              <details>
                ...
                <form action="<?= BASE_URL ?>/talento-humano/hojas-de-vida/subir" method="post" enctype="multipart/form-data" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px">
                  <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                  ...
                  <input type="date" name="fecha" required class="doc-input" style="flex:1 1 140px">
