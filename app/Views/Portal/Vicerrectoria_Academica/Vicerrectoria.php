...
$heroTitulo = $moduloTitulo ?: 'Vicerrectoría Académica';
$heroDesc = $moduloDesc ?: 'Programas académicos, reglamentos y documentos curriculares, organizados en un solo lugar.';
?>
...
<?php if ($carpetaActual): ?>
  <a href="<?= BASE_URL ?>/vicerrectoria-academica" class="doc-back">
    <i class="bi bi-arrow-left"></i> Volver a Documentos
  </a>
  <?php $rutaModuloActual = '/vicerrectoria-academica'; require ROOT_PATH . '/app/Views/Portal/_shared/_explorador_documental.php'; ?>

<?php else: ?>

  <div class="doc-search">
    <form action="<?= BASE_URL ?>/vicerrectoria-academica" method="get" class="doc-search-form" role="search">
      <i class="bi bi-search"></i>
      <input type="text" name="buscar" value="<?= e($terminoBusqueda) ?>" placeholder="Buscar programas, reglamentos, currículos...">
    </form>
    <?php $rutaModuloActual = '/vicerrectoria-academica'; require ROOT_PATH . '/app/Views/Portal/_shared/_agregar_documento.php'; ?>
  </div>

  <?php if ($terminoBusqueda !== ''): ?>

    <div class="doc-section-head"><h4>Resultados para "<?= e($terminoBusqueda) ?>"</h4>
      <a href="<?= BASE_URL ?>/vicerrectoria-academica" class="doc-clear-search">Limpiar búsqueda</a>
    </div>
          ...
          <div class="doc-archivo-row">
            ...
            <span class="doc-actions">
              <?php $urlVer = $a['origen'] === 'carpeta'
                  ? BASE_URL . '/vicerrectoria-academica/carpetas/descargar?id=' . (int) $a['id']
              ...
              <?php if (onlyoffice_configurado() && onlyoffice_editable($a['archivo'])): ?>
                <a class="doc-btn doc-btn--secondary doc-btn--icon" href="<?= BASE_URL ?>/editor?tipo=<?= $a['origen'] === 'carpeta' ? 'carpeta' : 'direccion' ?>&id=<?= (int) $a['id'] ?>&volver=<?= urlencode(BASE_URL . '/vicerrectoria-academica') ?>" title="<?= $esAdminDoc ? 'Editar' : 'Abrir' ?> dentro del portal"><i class="bi <?= $esAdminDoc ? 'bi-pencil-square' : 'bi-eye' ?>"></i></a>
              <?php endif; ?>
            </span>
          </div>
        <?php endforeach; ?>
  ...
  <?php else: ?>
    ...
    <?php if (!$subcarpetas): ?>
      <div class="empty-state">
        ...
        <p>Crea la primera categoría (Programas académicos, Reglamentos, Currículos...) para empezar a organizar los documentos académicos.</p>
        <?php if ($esAdminDoc): ?>
          <form action="<?= BASE_URL ?>/vicerrectoria-academica/carpetas/crear" method="post" class="doc-empty-create">
      ...
      <div class="doc-categorias">
        <?php foreach ($subcarpetas as $c): ?>
          <article class="doc-categoria-card">
            <a href="<?= BASE_URL ?>/vicerrectoria-academica?carpeta=<?= (int) $c['id'] ?>" class="doc-categoria-link">
            ...
            <?php if ($esAdminDoc): ?>
              <form action="<?= BASE_URL ?>/vicerrectoria-academica/carpetas/eliminar" method="post" onsubmit="return confirm('¿Eliminar esta categoría y TODO lo que tenga dentro?')">
                <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                <input type="hidden" name="direccion_id" value="<?= (int) $direccion['id'] ?>">
        ...
        <?php endforeach; ?>
        <?php if ($esAdminDoc): ?>
          <form action="<?= BASE_URL ?>/vicerrectoria-academica/carpetas/crear" method="post" class="doc-categoria-card doc-categoria-card--create">
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="direccion_id" value="<?= (int) $direccion['id'] ?>">
          ...
          <div class="doc-archivo-row">
            ...
            <span class="doc-actions">
              ...
              <?php if ($esAdminDoc && $a['origen'] === 'carpeta'): ?>
                <form action="<?= BASE_URL ?>/vicerrectoria-academica/carpetas/eliminar" method="post" onsubmit="return confirm('¿Eliminar este documento?')">
                  <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                  <input type="hidden" name="direccion_id" value="<?= (int) $direccion['id'] ?>">
            ...
            <div class="doc-archivo-row">
              ...
              <span class="doc-actions">
                <form action="<?= BASE_URL ?>/documentos/subir" method="post" enctype="multipart/form-data">
                  ...
                  <input type="hidden" name="volver" value="/vicerrectoria-academica">
