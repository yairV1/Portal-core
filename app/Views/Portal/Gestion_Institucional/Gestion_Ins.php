...
// el vacío mientras nadie los ha escrito (mismo criterio que Financiera.php).
$heroTitulo = $moduloTitulo ?: 'Gestión Institucional';
$heroDesc = $moduloDesc ?: 'Actas, resoluciones y documentos de gobierno institucional, organizados en un solo lugar.';
?>
...
<?php if ($carpetaActual): ?>
  <a href="<?= BASE_URL ?>/gestion-institucional" class="doc-back">
    <i class="bi bi-arrow-left"></i> Volver a Documentos
  </a>
  <?php $rutaModuloActual = '/gestion-institucional'; require ROOT_PATH . '/app/Views/Portal/_shared/_explorador_documental.php'; ?>

<?php else: ?>

  <div class="doc-search">
    <form action="<?= BASE_URL ?>/gestion-institucional" method="get" class="doc-search-form" role="search">
      <i class="bi bi-search"></i>
      <input type="text" name="buscar" value="<?= e($terminoBusqueda) ?>" placeholder="Buscar actas, resoluciones, manuales...">
    </form>
    <?php $rutaModuloActual = '/gestion-institucional'; require ROOT_PATH . '/app/Views/Portal/_shared/_agregar_documento.php'; ?>
  </div>
  ...
  <?php else: ?>
    ...
    <?php if (!$subcarpetas): ?>
      <div class="empty-state">
        ...
        <p>Crea la primera categoría (Actas de Consejo, Resoluciones, Manuales...) para empezar a organizar los documentos institucionales.</p>
        <?php if ($esAdminDoc): ?>
      ...
      <div class="doc-categorias">
        <?php foreach ($subcarpetas as $c): ?>
          <article class="doc-categoria-card">
            <a href="<?= BASE_URL ?>/gestion-institucional?carpeta=<?= (int) $c['id'] ?>" class="doc-categoria-link">
            ...
            <?php if ($esAdminDoc): ?>
              <form action="<?= BASE_URL ?>/gestion-institucional/carpetas/eliminar" method="post" onsubmit="return confirm('¿Eliminar esta categoría y TODO lo que tenga dentro?')">
                <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                <input type="hidden" name="direccion_id" value="<?= (int) $direccion['id'] ?>">
        ...
        <?php endforeach; ?>
        <?php if ($esAdminDoc): ?>
          <form action="<?= BASE_URL ?>/gestion-institucional/carpetas/crear" method="post" class="doc-categoria-card doc-categoria-card--create">
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="direccion_id" value="<?= (int) $direccion['id'] ?>">
          ...
          <div class="doc-archivo-row">
            ...
            <span class="doc-actions">
              ...
              <?php if ($esAdminDoc && $a['origen'] === 'carpeta'): ?>
                <form action="<?= BASE_URL ?>/gestion-institucional/carpetas/eliminar" method="post" onsubmit="return confirm('¿Eliminar este documento?')">
                  <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                  <input type="hidden" name="direccion_id" value="<?= (int) $direccion['id'] ?>">
...
<script src="<?= BASE_URL ?>/assets/portal/js/gestion-institucional.js"></script>
